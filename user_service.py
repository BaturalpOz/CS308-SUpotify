...
class UserService:
    ...
    def get_playlist_by_name(self, user_id: str, playlist_name: str) -> Optional[dict]:
        user = self.get_user_by_id(user_id)
        # Assuming each playlist is a string in this list
        playlist = next((p for p in user.playlists if p == playlist_name), None)
        if playlist:
            # If the playlist is found, return it as a dictionary with the name and an empty list of songs
            return {'name': playlist, 'songs': user.playlists.__getitem__(playlist_name)}  # Replace this with actual songs if available
        return None
